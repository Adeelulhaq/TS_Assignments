...
let users = [{ id: 1, pin: 1234, balance: 1000 },
...
function f_caheck_balance(p_uid) {
...
// Deposit  Amount  
function f_deposit_amt(p_uid, p_withdraw_amount) {
    let l_user_info = users.find(f_user => f_user.id === +p_uid);
    if (l_user_info) {
...
// Withdraw Amount  
function f_withdraw(p_uid, p_withdraw_amount) {
    let l_user_info = users.find(f_user => f_user.id === +p_uid);
    if (l_user_info) {
        l_user_info.balance = l_user_info.balance - p_withdraw_amount;
        return l_user_info.balance;
...
async function atm_main() {
    ...
    while (true) {
        ...
        // Check balannnce 
        if (l_transaction.Trantype === 'CB') {
            console.log('Your Current Balance is  ...: ', f_caheck_balance(user.id));
            let l_exit = await f_EXIT_FROM_ATM();
            if (l_exit === 'N') {
                console.log("Thank You for using Our ATM Services", '/n', 'Your Current Balance is', f_caheck_balance(user.id));
                break;
            }
        }
        // Exit from ATM Service
        else if (l_transaction.Trantype === 'E') {
            console.log("Thank You for using Our ATM Services", '/n', 'Your Current Balance is', f_caheck_balance(user.id));
            break;
        }
        // Withdrawl services
        else if (l_transaction.Trantype === 'W') {
            console.log('Your Current Balance is  ...: ', f_withdraw(user.id, await f_tran_Amount()));
            let l_exit = await f_EXIT_FROM_ATM();
            if (l_exit === 'N') {
                console.log("Thank You for using Our ATM Services", '/n', 'Your Current Balance is', f_caheck_balance(user.id));
                break;
        ...
        // Deposit  services
        else if (l_transaction.Trantype === 'D') {
            console.log('Your Current Balance is  ...: ', f_deposit_amt(user.id, await f_tran_Amount()));
            let l_exit = await f_EXIT_FROM_ATM();
            if (l_exit === 'N') {
                console.log("Thank You for using Our ATM Services", '/n', 'Your Current Balance is', f_caheck_balance(user.id));
                break;
