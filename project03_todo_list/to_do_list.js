...
let l_to_do_list = [{ task_id: 1, Task: "create Presentation on DAO", Task_Status: 'Pending' },
...
;
async function F_add_new_task(p_task, p_task_id) {
    l_to_do_list.push({ task_id: p_task_id, Task: p_task, Task_Status: 'Pending' });
}
async function f_show_all_list() {
    if (!l_to_do_list.length) {
        console.log("Their is no TODO task Available");
    }
    let todo = l_to_do_list.map((val) => {
        return { name: val.Task + '  ' + val.Task_Status, value: val.task_id };
    ...
    if (l_upd_del.upddel == 'U') {
        await f_update_task(p_task_id, await f_task_status());
    }
    if (l_upd_del.upddel == 'D') {
        await f_delete_task(p_task_id);
    }
}
async function f_update_task(p_task_id, // p_task : string , 
p_task_status) {
    let l_list = l_to_do_list.find(f_list => f_list.task_id === +p_task_id);
    if (l_list) {
        l_list.Task_Status = p_task_status;
    }
}
async function f_delete_task(p_task_id) {
...
async function f_to_do_main() {
    while (true) {
        ...
        if (l_to_do_option.Todo_option === 'CN') {
        ...
        else if (l_to_do_option.Todo_option === 'SA') {
            console.log('Show All TO DO tasks  ...: ');
            await f_show_all_list();
            console.log(l_to_do_list);
        }
        else if (l_to_do_option.Todo_option === 'E') {
            console.log('Thank you for using our services ...: ');
            await F_spinner('Exiting From TO DO App', 'Sucessfully Exit');
            break;
        }
    }
}
