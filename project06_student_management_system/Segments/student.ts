...
import { Student, Course } from './classes.js';

let sleep = () => new Promise((r) => setTimeout(r, 1000))

export async function F_ViewStudents(students: Student[], courses: Course[]) {
    if (!students.length) {
    ...
    if (index <= students.length - 1 && index >= 0) {
        ...
        student?.F_registerInCourse(course)
        student?.F_addStudentInCourseStudents(course)
        spinner.success({ text: chalk.greenBright(`Registered In Course Successfully and RS: ${course.fee} Fee Minus From Your Balance`) })
    }
}
export async function F_AddStudent(DetailsInputs: (type: string, name: string) => Promise<string | number>, students: Student[]) {
