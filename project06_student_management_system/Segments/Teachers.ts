import {  Course, Teacher } from './classes.js';
...
export async function F_ViewTeachers(teachers: Teacher[], courses: Course[]) {
    if (!teachers.length) {
    ...
    if (index <= teachers.length - 1 && index >= 0) {
        ...
        if (teacher?.courses.includes(course)) {
            spinner.error({ text: 'Course Is Already Assigned To Teacher ' })
            return
        }
        teacher?.F_assignCourse(course)
        teacher?.F_addTeacherInCourseTeacher(course)
        spinner.success({ text: "Course Assigned To Teacher Successfully" })
    }

}

export async function F_AddTeacher(DetailsInputs: (type: string, name: string) => Promise<string | number>, teachers: Teacher[]) {
