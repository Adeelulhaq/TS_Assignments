...
export async function F_ViewCourses(courses: Course[], teachers: Teacher[], students: Student[]) {
    if (!courses.length) {
    ...
    if (index <= courses.length - 1 && index >= 0) {
        ...
        if (choice === 'Add Student') {
            ...
            course?.F_registerStudent(student)
            course.F_addCourseInStudentCourses(student)
        ...
        if (choice === 'Assign Teacher') {
            ...
            course.F_setTeacher(teacher)
            course.F_addCourseInTeacherCourses(teacher)
...
export async function F_AddCourse(DetailsInputs: (type: string, name: string) => Promise<string | number>, courses: Course[]) {
